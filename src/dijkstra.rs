...
pub fn dijkstra(network: &mut GraphNetwork::<DijkstraNode, DijkstraArc>, start_node_id: NodeId) {
    ...
    loop {
        ...
        let current_network_node_distance = *network.mut_data_of_node(current_network_node_id).unwrap().distance;
...
pub fn network_factory(arcs: Vec<(NodeId,NodeId,usize)>) -> GraphNetwork::<DijkstraNode, DijkstraArc> {
    ...
    network.add_nodes(vec![DijkstraNode{distance: Box::new(usize::MAX), heap_id: usize::default()}; max_node_id+1].into_iter());
    network.bulk_connect(
        arcs.into_iter().map(|(from, to, weight)| (from, to, DijkstraArc::new(weight))),
    );
    network
}
